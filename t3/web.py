...
Here is the canonical "Hello, world" example app:
    ...
    import t3.httpserver
    import t3.ioloop
    import t3.web

    class MainHandler(t3.web.RequestHandler):
        def get(self):
            self.write("Hello, world")

    if __name__ == "__main__":
        application = t3.web.Application([
            (r"/", MainHandler),
        ])
        http_server = t3.httpserver.HTTPServer(application)
        http_server.listen(8888)
        t3.ioloop.IOLoop.instance().start()
...
from t3 import escape
from t3 import locale
from t3 import stack_context
from t3 import template
...
class RequestHandler(object):
    ...
    def render(self, template_name, **kwargs):
        ...
        for module in getattr(self, "_active_modules", {}).values():
            ...
            if embed_part: js_embed.append(embed_part)
            ...
            if embed_part: css_embed.append(embed_part)
            ...
            if head_part: html_heads.append(head_part)
            body_part = module.html_body()
            if body_part: html_bodies.append(body_part)
        if js_files:
    ...
    def get_login_url(self):
        ...
        """
        self.require_setting("login_url", "@t3.web.authenticated")
    ...
    def xsrf_token(self):
        ...
        if not hasattr(self, "_xsrf_token"):
            ...
            if not token:
                token = binascii.b2a_hex(uuid.uuid4().bytes).decode("utf8")
...
class FallbackHandler(RequestHandler):
    ...
    such as an Application or t3.wsgi.WSGIContainer.  This is most
    useful to use both t3 RequestHandlers and WSGI in the same server.
    Typical usage:
        wsgi_app = t3.wsgi.WSGIContainer(
